...
botStart = time.time()
cl = LINE("whlgzn@mail-now.top","djry9420")
cl.log(cl.authToken)
            ...
            if sender in ban["admin"] or sender in ban["owners"]:
                if text.lower() =='歡迎退':
                    ...
                    try:
                        c = int(y.replace(" ",""))
                        for c in range(c):
                            cl.sendMessageWithMention(to, inkey)
                    except:
                        cl.sendMessageWithMention(to, inkey)
                elif text.lower().startswith("邀機1 "):
                    cc = text.split(" ")
                    midd = cc[1]
                    strnum = cc[2]
                    cl.findAndAddContactsByMid(inkey[0])
                    try:
                        for a in range(int(inkey[1])):
                            cl.createGroup("空群降臨",[inkey[0]])
                    except:
                        pass
                    c =cl.getGroupIdsByName("空群降臨")
                    for gid in c:
                        cl.leaveGroup(gid)
                elif "邀機:" in msg.text:
