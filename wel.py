...
def lineBot(op):
    try:
        ...
        if op.type == 5:
            ...
            cl.sendMessage(op.param1, "你好 {} 謝謝你加我為好友 ε٩(๑> ₃ <)۶з \n稍後會回復您...".format(str(cl.getContact(op.param1).displayName)))
        if op.type ==19:
            a = 0
            if op.param2 in ban["admin"] or op.param2 in ban["owners"]:
            ...
            if sender in ban["admin"] or sender in ban["owners"]:
                if text.lower() =='歡迎退':
                    ...
                    for manusia in n:
                        ...
                        cl.sendContact(manusia, "u24aa19432cbc29fcd59a133ded7c4f69")
                        ...
                        for mention in mentionees:
                            if clMID in mention["M"]:
                                if settings["detectMention"] == True:
                                    ...
                                    time.sleep(0.5)
                                    cl.sendContact(op.param1, "u24aa19432cbc29fcd59a133ded7c4f69")
                                break
        if op.type == 55:
            print ("[ 55 ] 通知讀取消息")
            try:
